...
const Speech = (() => {
  ...
  const EV = typeof window !== 'undefined' ? window.__EMBED_VOICE : null;
  // inline (b64) or a file next to the page (src), fetched early so it is ready by the first cue
  const embedded = EV && (EV.b64 || EV.src) ? EV : null;
  let embeddedBytes = null;
  if (embedded && embedded.src && typeof fetch === 'function') {
    embeddedBytes = fetch(embedded.src).then((r) => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.arrayBuffer(); });
    embeddedBytes.catch(() => {});
  }
  let job = null;
  ...
  async function loadEmbedded(opts = {}) {
    ...
    if (!ctx) throw new Error('浏览器不支持 Web Audio');
    let data;
    if (embeddedBytes) data = (await embeddedBytes).slice(0);
    else {
