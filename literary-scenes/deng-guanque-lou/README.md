...
- **朗读默认关闭**。开启后使用浏览器自带的中文语音，逐句朗读，字幕按句高亮（只做句级同步，不声称逐字）。设备无中文语音时会如实提示，并可继续无声体验。
- 可在「设置 → 载入朗诵音频」导入自己有权使用的朗诵录音，页面会按停顿自动切分为「标题＋四句」并与章节同步。页面不附带任何第三方录音。
- **乐声默认关闭**。开启后为实时合成的古风配乐（古筝拨弦、箫、低音持续音、风声水声），音量较低，朗读时自动压低。
- 不需要任何 API Key。
